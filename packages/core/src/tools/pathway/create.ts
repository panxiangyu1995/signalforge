...
function parseSpecs(json: string, label: string): unknown[] | { error: string } {
  try {
...
function createEntities(
  ...
  for (const spec of specs) {
    ...
    if (spec.state_variables) {
      ...
      if (rawNode)
        updatePathwayData(rawNode, { stateVariables: parseStateVariables(spec.state_variables) })
    }
...
function createArcs(
  ...
  for (const spec of specs) {
    ...
    if (!sourceId || !targetId) {
      skipped.push(
        `${spec.source} → ${spec.target} (${!sourceId ? `source "${spec.source}" not found` : `target "${spec.target}" not found`})`
      )
      continue
    }
...
export const createPathway = defineTool({
  ...
  params: {
    ...
    entities: {
      type: 'string',
      description:
        'JSON array of entity specs: [{name, glyph_type, x?, y?, width?, height?, compartment?, state_variables?, clone_marker?}]',
      required: true
    },
    processes: {
      type: 'string',
  ...
  execute: (figma, args) => {
    const compartmentsResult = parseSpecs(args.compartments, 'compartments')
    if ('error' in compartmentsResult) return compartmentsResult
    const entitiesResult = parseSpecs(args.entities, 'entities')
    if ('error' in entitiesResult) return entitiesResult
    const processesResult = parseSpecs(args.processes, 'processes')
    if ('error' in processesResult) return processesResult
    const arcsResult = parseSpecs(args.arcs, 'arcs')
    if ('error' in arcsResult) return arcsResult

    const compartments = compartmentsResult as CompartmentSpec[]
    const entities = entitiesResult as EntitySpec[]
    const processes = processesResult as ProcessSpec[]
    const arcs = arcsResult as ArcSpec[]
    ...
    try {
      ...
      if (nameCollisions.length > 0) {
        result.warnings = [
          ...
          ...nameCollisions.map(
            (n) =>
              `Duplicate name "${n}" — later node shadows earlier, arcs may connect to wrong node`
          )
        ]
      }
